<template>
  <div class="fix-table-container">
    ...
    <template v-for="item in showColumns">
      ...
      >
        <el-checkbox v-if="col.label" v-model="col.show" @change="(val)=>handleChange(val,col.index)">
          {{ col.label }}
        </el-checkbox>
        ...
        <template v-if="col.attrs.active">
          <el-tooltip v-if="col.attrs.fixed!=='right'" content="固定在右边" effect="dark" placement="top">
            <svg-icon icon-class="td-right" style="cursor:pointer;margin-left: 8px;"
                      @click="handleFix(col.index,'right')"
            ></svg-icon>
          </el-tooltip>
          <el-tooltip v-if="col.attrs.fixed!=='left'" content="固定在左边" effect="dark" placement="top">
            <svg-icon icon-class="tuding" style="cursor:pointer;margin-left: 8px;"
                      @click="handleFix(col.index,'left')"
            ></svg-icon>
          </el-tooltip>
          <el-tooltip v-if="col.attrs.fixed" content="取消固定" effect="dark" placement="top">
            <svg-icon icon-class="guding" style="cursor:pointer;margin-left: 8px;"
